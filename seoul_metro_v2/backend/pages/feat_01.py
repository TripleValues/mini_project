...
def run_spark_feat01():
    try:
        ...
        df = df.withColumn("날짜_fixed", to_date(regexp_replace(F.col("날짜"), r"[^0-9]", ""), "yyyyMMdd")) \
               .withColumn("년도", year(F.col("날짜_fixed"))) \
               .withColumn("월", month(F.col("날짜_fixed")))

        row_total_expr = sum([F.col(c).cast("long") for c in time_cols])
