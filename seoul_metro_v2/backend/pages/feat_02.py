...
def create_table():
    ...
    CREATE TABLE IF NOT EXISTS db_metro.feat_02 (
        ...
        월 INT NOT NULL,
        일 INT NOT NULL,
        역명 varchar(10) not null,
        승차인원합계 BIGINT DEFAULT 0,
        하차인원합계 BIGINT DEFAULT 0,
        총인원합계 BIGINT DEFAULT 0,

        생성일 DATETIME DEFAULT CURRENT_TIMESTAMP,

        UNIQUE KEY uq_year_month (년도, 월, 일),
        INDEX idx_year_month (년도, 월, 일)
    );
    """
...
def feat_02_data_processing():
    query = """
    INSERT INTO feat_02 (년도, 월, 일, 역명, 승차인원합계, 하차인원합계, 총인원합계)
    SELECT
        YEAR(날짜) AS 년도,
        MONTH(날짜) AS 월,
        date(날짜) AS 일,
